import { BaseScraper } from './scraper/BaseScraper';
import { IndexPageScraper, IndexPageConfig } from './scraper/IndexPageScraper';
...
class TestScraper extends BaseScraper {
  ...
  public async testIndex(url: string) {
    ...
    class ConcreteIndexScraper extends IndexPageScraper {}
    const scraper = new ConcreteIndexScraper('https://www.bbc.co.uk', config);
  ...
  public async testDetail(url: string) {
    ...
    if (vocabHeader.length > 0) {
        ...
        parts.forEach((part, i) => {
            ...
            if (i < 6) console.log(`Part [${i}]: ${text}`);
        });
    } else {
